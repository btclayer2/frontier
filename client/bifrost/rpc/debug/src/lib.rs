	...
	fn handle_block_request(
		...
		return match trace_type {
			single::TraceType::CallList => {
				let mut proxy = fc_evm_tracing::listeners::CallList::default();
				proxy.using(f)?;
		...
		if let Some(block) = reference_block {
			...
			if let Some(transaction) = transactions.get(index) {
				let f = || -> RpcResult<_> {
					...
					Ok(fp_rpc_debug::Response::Single)
				};
				...
				return match trace_type {
					single::TraceType::Raw {
						...
						let mut proxy = fc_evm_tracing::listeners::Raw::new(
							disable_storage,
							disable_memory,
							disable_stack,
							raw_max_memory_usage,
						...
						proxy.using(f)?;
						Ok(Response::Single(
							fc_evm_tracing::formatters::Raw::format(proxy).ok_or(internal_err(
					...
					single::TraceType::CallList => {
						...
						let response = match tracer_input {
							TracerInput::Blockscout => {
								fc_evm_tracing::formatters::Blockscout::format(proxy)
									.ok_or("Trace result is empty.")
									.map_err(|e| internal_err(format!("{:?}", e)))
							}
							TracerInput::CallTracer => {
								let mut res = fc_evm_tracing::formatters::CallTracer::format(proxy)
									.ok_or("Trace result is empty.")
									.map_err(|e| internal_err(format!("{:?}", e)))?;
